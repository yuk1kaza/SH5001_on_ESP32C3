# SH5001 国产六轴惯性测量单元 ESP32-C3 Arduino 示例

本项目提供了在ESP32-C3微控制器上使用国产SH5001六轴惯性测量单元(IMU)的Arduino示例代码。同时，在配置代码的基础上加入了互补滤波算法，使得单片机可以直接输出四元数

## 硬件要求

- **微控制器**: ESP32-C3
- **传感器**: SH5001 六轴IMU (加速度计 + 陀螺仪 + 温度计)
- **通信接口**: I2C
...
```
原始数据格式：
ACC: 0.012 -0.003 0.998 g | GYRO: 0.5 -0.2 0.1 °/s | TEMP: 26.7 °C
四元数格式：
1.0 0.2 0.8 0.6
```
